...
<head>
<meta charset="UTF-8">
<title>Áreas do Design - Mapa Mental</title>
<style>
  html, body {
    margin: 0;
    padding: 0;
  ...
  /* Toolbar adaptado ao tema escuro */
.mm-toolbar:hover {--un-border-opacity:1;border-color:rgb(156 163 175 / var(--un-border-opacity));
  ...
  .mm-toolbar {
    background-color: rgba(40, 45, 56, 0.9) !important;
...
<div class="toolbar-container" style="position:fixed;right:16px;bottom:16px;"></div>

<script type="text/template" data-type="markmap">
---
markmap:
  colorFreezeLevel: 2
...
## Design Gráfico e de Comunicação
...
- Pilares
  ...
  - Público-Alvo (Quem)
  - Canal (Onde)
- Regra de Ouro: Forma segue a Função

## Design Digital e de Interação
...
## Design de Produto
- Bens de Consumo e Eletrónicos
- Mobiliário e Luminárias
- Equipamento Médico e de Saúde
- Design Automóvel e de Transportes
- Brinquedos e Jogos
- Pilares
  - Utilidade e Ergonomia
...
## Design de Moda
- Alta-Costura e Prêt-à-Porter de Luxo
- Fast Fashion e Mercado de Massa
- Design de Acessórios e Calçado
- Design Têxtil e de Estamparia
- Eco-Design e Moda Sustentável
- Pilares
- Documento Essencial: Ficha Técnica

## Design de Espaço
- Design de Espaços Residenciais
- Design Comercial e de Retalho
- Design de Espaços Corporativos
- Cenografia e Espaços Efémeros
- Design Expositivo e Museografia
- Design Biofílico (Integração com a Natureza)
- Pilares

## Design Estratégico e de Serviços
- Design de Serviços (Jornada do Serviço)
- Design Thinking (Pensamento Crítico)

## Pilares Fundamentais (Comuns a todos)
- Praticidade (Funciona?)
- Viabilidade (É possível produzir?)
- Desejabilidade (As pessoas querem?)
</script>

<!-- Bibliotecas markmap embutidas (sem dependência de rede) -->
<script>
  ...
  const _Markmap = class _Markmap {
    ...
    renderData(originData) {
      ...
      this.transition(circle).attr("r", 6).attr("cx", (d) => d.ySize - spacingHorizontal).attr("cy", (d) => d.xSize).attr("stroke", (d) => color(d.data)).attr(
        ...
        (d) => {
          var _a;
          return ((_a = d.data.payload) == null ? void 0 : _a.fold) && d.data.children ? color(d.data) : "#fff";
        }
      );
...
<script>
  window.addEventListener('load', () => {
    ...
    const tpl = document.querySelector('script[data-type="markmap"]').textContent;
    const { root } = transformer.transform(tpl);

    const mm = Markmap.create('svg.markmap', {
      color: (node) => {
        const palette = ['#5b6478', '#3a4a5c', '#2f4a3f', '#2f4a3f'];
        return palette[Math.min(node.depth || 0, palette.length - 1)];
      },
      paddingX: 16,
      spacingVertical: 8,
    ...
    if (Toolbar) {
      ...
      toolbar.el.style.right = '16px';
      toolbar.el.style.bottom = '16px';
